...
#       arr = size(u)
#       u_safe = zeros(arr[1],arr[2],arr[3])
...
#         for k in eachindex(u_safe[1,:,element])
#           # get safe density value 
#           ρ_safe = u_safe[1,k,element]
#           if ρ_safe < 0
#             error("safe value for density not safe")
#           end
#           α_p = beta * ρ_safe - u[1,k,element]
#           if (α_p > 0)
#             # avoid a divison by 0
#             p_dg = (u[1,k,element] - alpha[element] * u_safe[1,k,element]) / (1 - alpha[element])
#             if abs(u_safe[1,k,element]- p_dg) < tolerance
#               continue
#             end

#             tmp = α_p / (Δts * (u_safe[1,k,element]- p_dg))
#             cor = max(cor,tmp)
#           end
#         end
...
#           for k in eachindex(u[1,:,element])
#             println("--------------------------------------")
#             println("Dichte vorher $(u[1,k,element])")
#             println("Safe density; $(u_safe[1,k,element])")

#             # u[:,k,element] += cor * dt_2 * (u_safe[:,k,element]- u_dg[:,k,element]),
#              #u_dg = (u- α u_FV) / (1-α)
#             node_dg = (u[:,k,element] - alpha[element] * u_safe[:,k,element]) / (1 - alpha[element])
#             u[:,k,element] += cor * (u_safe[:,k,element]- node_dg[:])


#             println("Dichte nachher $(u[1,k,element])")
#             println("ap =  $(beta * u_safe[1,k,element] - u[1,k,element] )")
#             println("--------------------------------------")
#             println("")
#           end           
#         end
...
#         # pressure
#         for k in eachindex(u_safe[1,:,element]) # eachnode
#           tmp = 0.0
#           cor = 0.0
#           u_tmp = u

#           _, _, p_safe = cons2prim(u_safe[:,k,element], equations)
#           # if p_safe < 0
#           #   error("safe value for pressure not safe")
#           # end

#           _, _, p = cons2prim(u[:,k,element], equations) 
#           α_p = beta * p_safe - p
#           if (α_p > 0)
#             # Newton's method
...
#               node_dg = (u[:,k,element] - alpha[element] * u_safe[:,k,element]) / (1 - alpha[element])
#               du_dα =  Δts * (u_safe[:,k,element]- node_dg[:])
#               dp_du = (equations.gamma -1) * [0.5 * u[2,k,element]  -u[2,k,element]  1]
...
#               # calc new u 
#               u_tmp[:,k,element]  = u[:,k,element] + tmp * (u_safe[:,k,element]- node_dg[:])
#               # get pressure value
#               _, _, p = cons2prim(u_tmp[:,k,element], equations) 
#               α_p = beta * p_safe - p
#               if α_p <= 0
#                 break
#               end
...
#             end
#             for k in eachindex(u[1,:,element])
#               _, _, p_old = cons2prim(u[:,k,element], equations) 
#               println("--------------------------------------")
#               println("Druck vorher $p_old")
#               println("Safe value; $p_safe")
#               node_dg = (u[:,k,element] - alpha[element] * u_safe[:,k,element]) / (1 - alpha[element])
#               u[:,k,element] += cor * (u_safe[:,k,element]- node_dg[:])
#               _, _, p_new = cons2prim(u[:,k,element], equations) 
